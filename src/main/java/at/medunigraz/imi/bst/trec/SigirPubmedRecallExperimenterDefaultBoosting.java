...
public class SigirPubmedRecallExperimenterDefaultBoosting extends SuperSigirPubmedRecallExperimenter {
    public static void main(String[] args) {
        ...
        if (args.length != 4) {
            System.err.println("Parameters: <multi match mode> <match default operator> <word removal> <phrase slop>");
        }
        ...
        final String slop = args[3];
        templateProperties.put("phrase_slop", slop);


        final boolean wordremoval = Boolean.parseBoolean(args[2]);
        runExperiments(templateProperties, wordremoval, goldStandard, target, year, "", "--mmm:" + defaultMultiMatch + "-op:" + defaultOperator + "-wr:" + wordremoval + "-sl:" + slop);
    }
}
